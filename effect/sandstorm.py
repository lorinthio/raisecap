...
class Sandstorm(EffectBase):
    TUNING = EffectTuning.SANDSTORM

    def __init__(self, owner, level, duration, source, isRefresh):
        super(Sandstorm, self).__init__(owner, duration, isRefresh)
        self.level = level
    ...
    def addStorm(self, owner):
        tuning = SkillTuning.combine(SkillTuning.ENSHROUD_FORM, 'ENSHROUD_FORM_SILVAN')
        powerUp = True if self.duration == SkillTuning.ENSHROUD_FORM.POWER_UP_DURATION else False
        self.storm = EnshroudForm._createStormEntity(owner, self.level, 'enshroud_silvan', tuning, powerUp)
        for effect in [EffectTuning.FIRESTORM.NAME, EffectTuning.WINDSTORM.NAME, EffectTuning.SNOWSTORM.NAME]:
    ...
    @staticmethod
    def register():
        game.effects.register(Sandstorm.TUNING.NAME, Sandstorm)
