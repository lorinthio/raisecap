...
class Snowstorm(EffectBase):
    TUNING = EffectTuning.SNOWSTORM

    def __init__(self, owner, level, duration, source, isRefresh):
        super(Snowstorm, self).__init__(owner, duration, isRefresh)
        self.level = level
    ...
    def addStorm(self, owner):
        tuning = SkillTuning.combine(SkillTuning.ENSHROUD_FORM, 'ENSHROUD_FORM_LACIES')
        powerUp = True if self.duration == SkillTuning.ENSHROUD_FORM.POWER_UP_DURATION else False
        self.storm = EnshroudForm._createStormEntity(owner, self.level, 'enshroud_lacies', tuning, powerUp)
        for effect in [EffectTuning.FIRESTORM.NAME, EffectTuning.WINDSTORM.NAME, EffectTuning.SANDSTORM.NAME]:
    ...
    @staticmethod
    def register():
        game.effects.register(Snowstorm.TUNING.NAME, Snowstorm)
